...
- **決まった間隔での自動実行は入れない**（Issue #107 で討論して決めた）。週1回のこの手順にぶら下げるのが、いま存在する唯一の定期作業だから。定時に走らせる仕組み（Netlify・GitHub Actions・Mac の launchd）は、どれも失敗が誰にも届かない
...
- **銘柄とテーマを消したときは、テーマ所属も一緒に消えている。** 消えたテーマ所属は、それぞれ別の削除の行として監査ログに残っている。親（銘柄・テーマ）を先に戻してから、上の SQL の `event` を `theme_stock` に替えて1行ずつ戻す（開発用DBで戻ることを確かめた。2026-09-17）
