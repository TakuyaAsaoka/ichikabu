...
> **この段落はもう当たらない。** iOS のログインは消えた（→ [ログイン廃止 設計書](2026-08-14-86-no-login-design.md) §5）。`/api/auth/*` を叩くのは管理UIだけで、iOS が使うのは `/api/events`・`/api/stocks`・`/api/health` の3本。よって**この節のレート制限の話は、すべて管理UIの利用者3人だけに関わる**。
...
2行目が決め手。**既定は、クライアントが書いたヘッダでは鍵を動かせない。**全員で1つの枠になるのは、緩いのではなく「回り道ができない」という意味で厳しい。利用者は3人なので、共有でも足りる。
...
**やり直す条件**: 本番の `rate_limit` テーブルに `no-trusted-ip|` で始まる `key` が実際に出ているとき。共有の枠だと確定するので、書き換えられる危険を引き受けてでもヘッダを足す判断に変わりうる。確かめ方は `server/` で次を実行する（`.env.deploy.local` が要る。→ `docs/guides/deploy.md`）。
