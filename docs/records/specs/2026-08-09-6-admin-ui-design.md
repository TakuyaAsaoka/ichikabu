...
- 対応 Issue: [#6 管理UI: 銘柄・保有・テーマの登録](https://github.com/TakuyaAsaoka/ichikabu/issues/6)
...
**Issue #6 の対象から theme / theme_stock の登録画面を外し、別Issueに切り出す。** 全体設計書 §5.1 で出典を「各社IRページ1つだけ」に絞った結果、テーマイベントが登録できない。テーマの登録画面を作っても紐づけ先のイベントが無く、動作確認もできない。出典を増やしてテーマイベントが登録できるようになった時点で作る。

**Issue #6 の本文と完了条件はこの決定に合わせて書き換える必要がある。** 現在の本文は theme / theme_stock を「やること」に含み、完了条件にも「テーマ・テーマ所属も同様に」の項がある。実装に入る前に本文を直す（→ §11 順1）。
...
| `sonner`（トースト通知） | エラーはフォームの直下に出す。`useActionState` の戻り値をそのまま表示するほうが素直で、JS が無くても動く |
...
| 対象 | 理由 |
|---|---|
| theme / theme_stock の登録画面 | 別Issueに切り出す（→ §2） |
| イベントの登録画面 | Issue #7 |
