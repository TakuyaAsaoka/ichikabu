...
2. Keychain を空にして起動するとサインイン画面が出る（アプリを消して入れ直すだけでは Keychain の項目は消えない（iOS の仕様）ため、`xcrun simctl keychain booted reset` で消す）
3. サーバー側のセッションを消してから開き直すと、401 でサインイン画面に戻る
