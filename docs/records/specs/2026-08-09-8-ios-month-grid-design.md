...
| 4 | カレンダーの日付計算は JST 固定（グレゴリオ暦・日曜始まり・`Asia/Tokyo`）。日付キーと月の表題は、`Calendar` から取り出した年月日の成分から組み立てる（`DateFormatter` は使わない） | イベントの日付は契約上 JST（`openapi.yaml` の `time` の説明）。端末のタイムゾーンでセルの日付がずれてはならない。`DateFormatter` を使わない理由は2つ。①`Sendable` ではないので `static` に持てず、Swift 6 の同時実行チェックで警告になる。②成分から組み立てれば、ロケールの影響を受ける経路がそもそも無い（端末の暦設定が和暦でも `yyyy` の値がずれない。ロケールを指定し忘れる余地も無い） | `Calendar.current` を使う（端末設定に依存する）／`DateFormatter` に `en_US_POSIX` ロケールを固定して使う（`Sendable` ではないため `static` に持てず、Swift 6 の同時実行チェックで警告になる） |
...
> **Issue #157 で変えた（2026-09-16）。** 開発用DBにだけ、テーマ `自動車` と 7203 の所属を1件ずつ入れる（`seedSampleTheme`）。管理画面の「テーマの編集」「テーマ所属を外す」を撮るため。上の理由のうち「テーマイベントを登録できない」は、今は当てはまらない（全体設計書 §5.1 の表の下）。
> **本番DBには入れない。** seed が本番に入れてよいのは出典のある事実（銘柄・イベント）だけで、どの銘柄をどのテーマに入れるかは運用者が決める値だから。本番に入ると、監査ログに残らないまま `/api/stocks` の `themeIds` が変わり、そのテーマにイベントを付けた日から iPhone の表示に効く。開発用DBかどうかは接続先のホスト名（`localhost`・`127.0.0.1`）で見る。環境変数のフラグは採らない。`docs/guides/deploy.md` §5 の手順ではシェルに無い値が `.env.local`（開発用）から読まれ、本番でもフラグが立つため

| 日付 | 種別 | `short_label` | ★ | 出来事と出典（`source_url` に入れる） |
|---|---|---|---|---|
| 2026-08-04 | 銘柄 7203 | `7203決算` | 3 | トヨタ自動車 2027年3月期 第1四半期決算（[global.toyota の決算要旨PDF](https://global.toyota/pages/global_toyota/ir/financial-results/2027_1q_summary_jp.pdf)。表題に「2026年８月４日」「2027年３月期第１四半期決算要旨」の記載あり） |
...
| 2026-08-04 | 銘柄 6367 | `6367決算` | 1 | ダイキン工業 2027年3月期 第1四半期決算（[daikin.co.jp のニュースリリース](https://www.daikin.co.jp/press/2026/20260804)。表題「2027年3月期 第1四半期 決算短信」・発表日2026年8月4日の記載あり） |
...
3. 8月のページ（起動月が8月でなければ左右のスワイプで8月に移動する）で、8/4 のセルに `7203決算`（赤線＋太字）＋ `9434決算`（線なし）＋ `+1` が出ることを確認する
4. 8/4 以外の日にイベントが出ていないこと、実行日の日番号に丸背景が付いていることを確認する
5. 左右のスワイプで前月・翌月に移動でき、2025年8月〜2027年8月の端で止まることを確認する

結果（`xcrun simctl io booted screenshot` で撮った画面で確認した）:

| 見たもの | 結果 |
|---|---|
| 月の表題 | `2026年8月`。`NavigationStack` を外したがステータスバーには張り付いていない |
| 曜日ヘッダ | 日・月・火・水・木・金・土 |
| 8/4 のセル | `7203決算`（左端に赤の細線・太字）＋ `9434決算`（線なし・太字でない）＋ `+1` |
| ラベルの色 | 2件とも琥珀（銘柄）。★3と★2で色は変わっていない |
| 8/4 以外の日 | イベント無し |
| 8/9（実行日）の日番号 | 丸背景あり |
| 埋め草（7/26〜8/1・9/1〜9/5） | 日番号がグレー、イベント無し |
| 行数 | 6週42セル |
| 左右のスワイプ | 前月・翌月に移動し、端で止まる |

**この目視でサインインの不具合を1件見つけて直した（コミット `b04d2a3`）。** `APIClient` が `URLSession.shared` を使っていたため、サインインの応答に付く `better-auth.session_token` が保存され、以降の全リクエストに付いていた。Better Auth は Cookie が付いた要求にだけ Origin を検査する（`origin-check.mjs` の `useCookies`）ため、Origin を送らないアプリからのサインインが `MISSING_OR_NULL_ORIGIN` で 403 になる。Issue #5 の確認時に成功したのは1回目だけ Cookie が無かったためで、その成功が2回目以降を壊していた。このアプリは Bearer トークンで認証する（全体設計書 §9）ので、Cookie を保管しない専用の `URLSession` を持たせた。
