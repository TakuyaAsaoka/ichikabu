...
| 11 | 確認用データの日付は、一次情報で確認した実在の日付だけを使う（→ §3） | 全体設計書 §5.1 の出典ポリシーは本番データだけの話ではない。作った日付を seed に置くと、後から「この日付は何だったのか」を誰も判定できなくなる。実在の日付なら `source_url` に出典を残せ、Issue #7 のイベント登録の動作確認にもそのまま使える | それらしい日付を作る（初案は9/16にトヨタの決算を置いていたが、決算月が3月のトヨタに9月の決算は無い） |

## 3. 確認用データ（server）

イベントを届けるには保有が要る（`GET /api/events` は保有起点で絞る）ため、最小のマスタごと seed に足す。

**日付は実在するものだけを使う**（全体設計書 §5.1 の出典ポリシー）。決算発表日は発表の約1か月前にしか確定しないため（全体設計書 §14 #6 の根拠）、今日（2026-08-09）の時点で確定しているのは8月上旬までの分になる。よって確認用の山は**起動月である8月**に置く。副産物として、目視確認の主要部分がスワイプ無しで行える。
...
| 日付 | 種別 | `short_label` | ★ | 実在の出来事と出典（`source_url` に入れる） |
|---|---|---|---|---|
| 2026-08-03 | テーマ（自動車） | `新車販売` | 1 | 自販連・全軽自協が2026年7月の新車販売台数を発表（[全軽自協](https://www.zenkeijikyo.or.jp/statistics/4soku)） |
...
★（`importance`）は運用者の主観の設定値なので、実在性の対象外。3件を★3・★2・★1に散らして、セル上の差が「★3かそれ以外か」の2値になることを確認できるようにする。
...
| ページ | セル | 見えるもの | 対応する完了条件 |
|---|---|---|---|
| 8月（起動月） | 8/4 | `7203決算`（琥珀・赤線＋太字）＋ `9434決算`（琥珀・線なし）＋ `+1` | ラベル表示・`+N`・★3の形・★1と★2に差がない |
| 8月（起動月） | 8/3 | `新車販売`（紫） | テーマの色 |
| 9月（右へ1スワイプ） | 9/16・9/17 | `FOMC`（藍・赤線＋太字）が2日とも | 市場の色・期間イベントの各日表示・スワイプ |

3色（琥珀・紫・藍）が揃うのは2ページにまたがるが、そのスワイプ自体が完了条件「横スワイプで前月・翌月に移動できる」の確認を兼ねる。

なお既存 seed の米CPI（9/11）・米雇用統計（10/2）は BLS の公表スケジュールを一次情報で確認できていない（bls.gov が 403 を返す）。今回は触らず、確認は Issue #7 のイベント登録に回す。FOMC の 9/16〜9/17（JST）は確認済みで、[FRB の公表日程](https://www.federalreserve.gov/monetarypolicy/fomccalendars.htm)の9月15〜16日（米東部時間、発表は16日14:00 ET＝JST 9/17 03:00）と一致する。
...
実装後は初回だけ `イベントを 4 件作成した` になり、2回目以降は 0 件に戻ることを確認する（未実施）。
...
2. 起動月（8月）のページで、8/4 に `7203決算`（赤線＋太字）＋ `9434決算`（線なし）＋ `+1`、8/3 に `新車販売`（紫）が出ることを確認する
3. 右へ1スワイプして9月を出し、9/16 と 9/17 の両方に `FOMC`（藍・赤線＋太字）が出ることを確認する
