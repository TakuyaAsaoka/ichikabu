...
それ以外の形は毎回関数を起動する。次の3つは**本番で実測した**（2026-08-16）。
...
**もう1つ、`GET /api/events?_rsc=1`・`?_rsc=2` でも鍵が割れる**（→ §2）。
こちらは `dist/run/headers.js` を読んで導いたもので、**本番では叩いていない**。
叩き方は Issue #120 のコメントに置いた。

どれも1件あたり0.0038のままなので、この形で叩かれると7.9万件で無料枠が尽きる。

上の3つは #118 のスコープ（公開API2本をCDNに載せる）の外である。
`?_rsc=N` は対象の経路そのものだが、塞ぐ手立てがランタイム側にあり
（こちらのヘッダでは動かせない）、打つ手が上の3つと同じ場所になる。
まとめて Issue #120 に置いた。
