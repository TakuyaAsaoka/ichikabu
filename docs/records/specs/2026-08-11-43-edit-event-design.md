...
| やらないこと | 理由 |
|---|---|
| 銘柄・テーマの編集と削除 | 別Issueに起こす。規約の条件がかからないため急がない。削除は外部キーで弾かれる場合がある（銘柄は保有・イベントから、テーマはイベントから `restrict` で参照される）ので、そのIssueで日本語のエラー文を足す。→ Issue #67 で実施（[銘柄とテーマの編集・削除 設計書](2026-08-12-67-edit-stock-theme-design.md)） |
