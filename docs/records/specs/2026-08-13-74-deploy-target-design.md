...
当時は「判断材料を取るための足場」と位置づけていたが、これがそのまま本番になった。無料プランをいつ上げるかは Issue #16 で決めた（→ `docs/guides/deploy.md` §7・`docs/guides/backup.md` §4）。
...
| 本番を今の Netlify + Supabase から載せ替える | 保留。無料プランの上限に着いたときの打つ手は「有料プランに上げる」であって載せ替えではない（→ `docs/guides/deploy.md` §7） |
