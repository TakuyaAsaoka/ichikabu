...
| 出すもの | 4種類（作ったときは5種類。#179 で1つ消した。→ §3・§3.2） |
...
| ~~出典の表示名なし~~ | ~~`source_url` はあるが `source_name` が無い行~~。**Issue #179 で消した**（→ §3.2） |
...
- 直しに行ける画面があるものだけリンクを持つ。「決算月なし」は `/stocks/[id]`、「過ぎた非アクティブ」は `/events/[id]`。「次の決算日が未登録」（登録は `/` のフォーム）と「休場日リストの不足」（直すのはソースコード）はリンクを持たない
...
### 3.2 「出典の表示名なし」は消し、入力の側で止めた（Issue #179）

**この判定は、直すべき行を拾わず、正しい行だけを拾っていた。**

| 行 | 判定の結果 | 本当は |
|---|---|---|
| 各社IRの決算イベント（URLだけ） | 出た（見本の DB の3件すべて） | 正しい形。各社IRは出典の記載が条件でない（[出典表示 設計書](2026-08-11-41-event-source-design.md) §3.1） |
| 出典の無い市場イベント（府省・FRB・BLS の日付） | 出なかった | 規約の条件を満たさない。`GET /events` が出典を返さないままアプリに出る |

直し方は2つあり、別々のエージェントに立場を分けて3回討論させて決めた。

| 案 | 採否 | 理由 |
|---|---|---|
| DB の CHECK `event_market_source_check`（`market IS NULL OR source_name IS NOT NULL`）で入力時に止め、この種類を消す | **採った** | 全体設計書 §5 の表が、市場イベントの出典は「出典をシートに出すのが条件」と明文で決めている。後から知らせる形では、状態の画面を開くまでの間、条件を満たさない行がアプリに出る。止めてしまえば拾う行が残らない |
| 判定を「市場イベントで `source_name` が無い行」に変える | 採らない | 上の「開くまでの間」を防げない。記載の要らない出典を市場イベントに使い始めると、正しい行が抜けとして出続ける（#179 の症状が戻る） |

- 採った案の代償は、市場イベントを出典なしで作っていたテストの見本の書き換え（`test/inputs.ts` の `MARKET_SOURCE`）。テストが壊れることは設計を緩める理由にしない
- 本番のマイグレーションは1つのトランザクションで流れる（drizzle の `migrate`）。出典の無い市場イベントが本番に残っていれば、制約の追加だけが失敗して巻き戻り、行は変わらない。その行を直してから流し直す
- 前提が崩れるのは出典を増やすとき。全体設計書 §5.1「出典を増やすときの手順」の2に、この制約を見直すことを書いた
