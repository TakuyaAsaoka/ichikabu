 ...
 * DBは正しい順を返すことも間違った順を返すこともでき、実際に両方を見た。
 * `desc(auditLog.id)` を消しても走らせるテストは緑のままになる（実測）。
...
export function recentQuery() {
  return (
    db
      ...
      // 実行計画次第で変わる。開発DBでは `created_at` だけで並べたとき
      // `2,3,4,5,1` や古い順が返るのを見た。**この崩れは走らせるテストでは
      // 捕まえられない**（実行計画によって正しい順が返ってしまう）ため、
