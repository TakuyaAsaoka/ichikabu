...
export const event = pgTable(
  ...
  (t) => [
    ...
    // 出典（府省・FRB・BLS）は、どれも出典の記載が利用の条件で（全体設計書 §5・§5.1）、
    // 名前が無いと GET /events が出典を返さない。状態の画面で後から拾う形は、開くまでの間
    // 条件を満たさないままアプリに出るため採らなかった（Issue #179）。
    // 出典の記載を求めない出典を市場イベントに使い始めるときは、§5.1「出典を増やすときの
    // 手順」の2でこの制約を見直す
    check(
