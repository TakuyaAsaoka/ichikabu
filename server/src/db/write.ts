...
export async function upsertMarketEvents(
  ...
  await db.transaction(async (tx) => {
    ...
    // 一度に2件以上消えるのは、中止・延期ではなく公表予定の読み落としである
    // （Issue #107 で討論して決めた）。上の0件のときの守りは、XML が
    // **全部**読めなくなったときにしか効かない。属性が1つ増えただけで
    // 15件中9件しか読めなくなる形の変化が実在し（実測）、そのとき残りの
    // これからの回がまとめて非アクティブになる。投げると取り引きごと巻き戻り、
    // 登録も更新も残らない。
    //
    ...
    if (deactivated.length > 1) {
      throw new Error(
        `非アクティブにする回が多すぎる（${deactivated.length}件）。公表予定を読み落とした可能性がある: ${deactivated.join(" / ")}`,
      );
    }
  });
  return { created, changed, deactivated, entries };
