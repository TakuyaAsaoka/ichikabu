...
describe("銘柄の編集画面", () => {
  ...
  it("所属しているテーマの一覧と、その件数を出した確認の文が出る", async () => {
    ...
    //
    // 題材の「半導体」「防衛」は、この画面の並び順（`theme.name` の照合順序）が
    // どれであっても前後が入れ替わらない2語を選んでいる（開発用DBが持つ
    // 照合順序 879 件を総当たりして、逆になるものが0件と確かめた）。
    // なお `theme.name` 順は五十音順ではない。「宇宙」「旅行」のように
    // 照合順序で前後が動く語をここへ足すと、実装が正しくても赤くなる
