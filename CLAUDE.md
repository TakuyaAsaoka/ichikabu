...
製品の判断（作るもの・作らないもの・収益化・投資助言との境界）は `docs/reference/product.md`、出典の可否と取り方は `docs/reference/source-terms.md`。コードの決まりは、この文書とコードの隣のコメントにある。
...
- **1リポジトリにする理由**: Swift と TypeScript の間では型を共有できないため、契約のズレを人が捕まえる手段は「同じ PR で両側の差分が並んで見える」ことしか残らない
- **持ち込まないもの**: Turbo・tRPC・SwiftLint・サーバー実装から OpenAPI を作る仕組み・契約テスト（型が合っていて実行時の値だけ違うズレは捕まえない。エンドポイントが少なく割に合わない）
- **ORM は Drizzle**。`event` の対象3列の排他（`CHECK (num_nonnulls(...) = 1)`）をスキーマの定義にそのまま書けるため（Issue #16）。DB が PostgreSQL なのも同じ CHECK に頼るため

## 品質ゲート

GitHub Actions の CI は使わない。**ローカル検証がマージ前の唯一のゲート**。Archive と App Store への提出はゲートに入れない（十数分かかる）。
...
- **`table.tsx` は入れて、使わないので消した**（#161）。イベント一覧を表にする案は、720px に収めるのに一覧から項目を落とす必要があり、「一覧に出典を出す」と衝突したため見送った
- **イベント一覧は、PC（`lg` 以上）だけ `grid` と `subgrid` で列をそろえ、1件を1行に収める**（#172。`server/app/(signed-in)/events/page.tsx`）。項目は落とさず、名称と出典だけを切り詰める。一覧から名称と出典を外す案は、「一覧に出典を出す」を覆すので採らなかった
...
**フォーカスの輪は `ring` の色で、透かさずに描く。** 描き方は要素で2通りある（#176）。
...
- `openapi-typescript` の生成物（`server/src/generated/`）は**コミットしない**。品質ゲートで毎回 `pnpm gen` が走るため、コミットしても常に再生成される冗長なファイルになるうえ、契約とのズレを生む余地しかない。iOS側の生成物も同様（ビルドのたびに作り直されるので、コミットし忘れが起きない）
