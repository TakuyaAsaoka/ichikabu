...
- 片方のWorktreeで `compose.yaml` のイメージを書き換えて `docker compose up -d` すると、共有しているコンテナがそのイメージで作り直される。まだ取り込んでいないWorktreeでは `test/image-tag.test.ts` が赤くなるので、そのブランチで最新の main を取り込む
